...
class Config:
    ...
    def _create_topography_layer(
        ...
        if topo_type == "operational":
            ...
            fn_name = None
            kwargs = None
        elif topo_type == "manual":
            y_coords = np.arange(self.yaml_data["area"]["screen_size"][0]).reshape(-1, 1)  # Height
            x_coords = np.arange(self.yaml_data["area"]["screen_size"][1])  # Width
            topo_grid = x_coords + y_coords
            topo_layer = ManualTopographyLayer(
                # FUNCTION GET FROM PYTHON
                topo_grid
            )
                #np.random.randint(1000, 1300, size=(
                #    self.yaml_data["area"]["screen_size"][0],
                #    self.yaml_data["area"]["screen_size"][1]))
