...
class ManualTopographyLayer(TopographyLayer):
    """
    Layer that stores elevation data computed from a function.
    """
    ...
    def __init__(self, elevation_grid: np.ndarray) -> None:
        ...
        """
        super().__init__()
        self.height = elevation_grid.shape[0]
        self.width = elevation_grid.shape[1]

        self.data = self._make_data(elevation_grid)
        self.contours = self._make_contours()

    def _make_data(self, elevation_grid: np.ndarray) -> np.ndarray:
        """
        Use self.elevation_fn to make the elevation data layer.
...
class ManualFuelLayer(FuelLayer):
    """
    Layer that stores fuel data computed from a function.
    """
